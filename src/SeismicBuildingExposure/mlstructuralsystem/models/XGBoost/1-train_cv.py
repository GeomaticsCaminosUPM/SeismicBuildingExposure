...
def build_xgb(num_classes, params):
    correct_types = {}
...
def main():
    ...
    for config_name, cfg in EXPERIMENT_CONFIGS.items():
        for grid_params in ParameterGrid(XGB_GRID):
            ...
            with mlflow.start_run(run_name=run_name):
                ...
                for fold, (tr_idx, val_idx) in enumerate(skf.split(X, y), 1):
                    ...
                    model      = build_xgb(N_CLASSES, grid_params)
                    fit_kwargs = {}
                    ...
                    fold_path = MODELS_DIR / f"xgb_{config_name}_fold{fold}.pkl"
                ...
                for fold in range(1, config.N_SPLITS + 1):
                    pth = MODELS_DIR / f"xgb_{config_name}_fold{fold}.pkl"
                    if pth.exists():
                        pth.unlink()
