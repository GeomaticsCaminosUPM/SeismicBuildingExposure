...
"""TODO: Direction computation for slenderness with bounding box"""
...
def inertia_slenderness(geoms:gpd.GeoDataFrame|gpd.GeoSeries, return_direction=False):
    """
    ...
    if return_direction:
        ...
        inertia_df = calc_inertia_principal(
            geoms, 
            principal_dirs=True
        )
        ...
        slenderness_angle = np.arctan2(vect_1[:,1], vect_1[:,0])  # in radians
        return list(slenderness_result), list(slenderness_angle)
    else:
        I_max, I_min = calc_inertia_principal(geoms.geometry,principal_dirs=False)
        return list(np.sqrt(I_max / I_min))

def circunscribed_slenderness(geoms:gpd.GeoDataFrame|gpd.GeoSeries, return_direction=False):
    geoms = geoms.copy() 
    geoms = geoms.reset_index(drop=True)
    if type(geoms) is gpd.GeoSeries:
        geoms = gpd.GeoDataFrame({},geometry=geoms.geometry,crs=geoms.crs)
    ...
    slenderness = list(np.maximum(np.array(total_length_1) / np.array(total_length_2),
    ...
    if return_direction:
        ...
        vect_1 = np.array([*df['vect_1']])
        slenderness_angle = np.arctan2(vect_1[:,1], vect_1[:,0])  # in radians
        return list(slenderness), list(slenderness_angle)
    else:
        return slenderness
    
def min_bbox_slenderness(geoms: gpd.GeoDataFrame | gpd.GeoSeries, return_direction=False):
    geoms = geoms.copy()
    geoms = geoms.reset_index(drop=True)
    ...
    # Slenderness ratio
    slenderness = list(
        np.maximum(np.array(L1) / np.array(L2),
    ...
    # Get rectangles
    rectangles = gpd.GeoSeries(min_bbox(geoms, return_lengths=False), crs=geoms.crs)

    # Get direction vectors
    dir_1_x, dir_1_y, dir_2_x, dir_2_y = rectangle_to_directions(rectangles, normalize=True)

    # Compute angles (in degrees)
    angle_1 = np.degrees(np.arctan2(dir_1_y, dir_1_x))
    angle_2 = np.degrees(np.arctan2(dir_2_y, dir_2_x))

    angles = list(np.where(
        np.array(L1) > np.array(L2),
        angle_2,
        angle_1
    ))

    return slenderness, angles
...
def get_a(geoms:gpd.GeoDataFrame|gpd.GeoSeries) -> list:
    geoms = geoms.copy()
    geoms = geoms.reset_index(drop=True)
    # Ensure the geometries are in a projected CRS for accurate area and length calculations
    if not geoms.crs.is_projected:
