...
import SeismicBuildingExposure.MLstructural_system.dataset as dataset 
...
def get_best_run_and_params(experiment_name):
    runs = mlflow.search_runs(
        ...
        filter_string="params.model_type = 'CatBoost' and attributes.status = 'FINISHED'",
        output_format="pandas"
    )
    if len(runs) == 0:
        raise RuntimeError(
            f"No completed CatBoost runs found in '{experiment_name}'. "
    ...
    catboost_keys = {"depth", "learning_rate", "iterations"}
    best_params = {k: safe_cast_param(v) for k, v in all_params.items() if k in catboost_keys}
...
def main():
    ...
    # Aplicar SMOTE antes de calcular class_weights (sobre datos ya resampleados)
    if USE_SMOTE:
    ...
    with mlflow.start_run(
        ...
        mlflow.catboost.log_model(model, name=f"model_final_{MODEL_TYPE}")
