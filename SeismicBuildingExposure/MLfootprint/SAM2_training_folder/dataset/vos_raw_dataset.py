...
class COCORawDataset(VOSRawDataset):
    def __init__(
        self,
        anns_file,
        img_folder=None,
        ...
        if img_folder:
            ...
            self.img_folder = os.path.dirname(anns_file)
    ...
    def get_video(self, idx):
        ...
        segment_loader = COCOSegmentLoader(
            anns,
            mask_area_frac_thresh=self.mask_area_frac_thresh,
            image_path=img_path,
            uncertain_iou=self.uncertain_iou,
        ...
        frames = []
        for frame_idx in range(self.num_frames):
            frames.append(VOSFrame(frame_idx, image_path=img_path))

        # video id needs to be image_id to be able to load correct annotation file during eval

        video = VOSVideo(str(image_id), int(image_id), frames)
